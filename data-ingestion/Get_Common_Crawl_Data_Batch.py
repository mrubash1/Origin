...
import warc, boto, json, ast, re, os
...
def Iterator_for_opening_files(rootdir, input_directory, output_directory):
    ...
    for dirs, subdirs, files in os.walk(input_directory):
        for file in files:
            if file != '.DS_Store': #Errors keep on being thrown from this file
                print 'THIS FILE WILL BE RUN: ', str(file)
                with open(input_directory+'/'+str(file), 'r') as f: #file to read
                    try:
...
output_directory = rootdir + '_upload_to_s3'
...
#RUN EVERYTHING
Iterator_for_opening_files(rootdir, input_directory, output_directory)
